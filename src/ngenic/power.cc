...
/** original Gadget4 use the approxmate form to calculate D1 D2 f1 f2 */
/**
...
double ngenic::ngenic_growth(double a)
{
  /** D1 without renormalization */
  ode_soln soln = growth_ode_solve(a);
  // ode_soln soln_a1 = growth_ode_solve(1);
  // return soln.y0 / soln_a1.y0;
  return soln.y0;
}
...
{
  ode_soln soln = growth_ode_solve(a);
  /* f = d log D / d log a. Note soln.y1 is d d1 / d log a */
  return soln.y1 / soln.y0;    
}
...
double ngenic::ngenic_f2_omega(double a)
{
  ode_soln soln = growth_ode_solve(a);
   /* f = d log D / d log a. Note soln.y1 is d d1 / d log a */
  return soln.y3 / soln.y2; 
}

double ngenic::ngenic_Phi2fac(double a)
{
  ode_soln soln = growth_ode_solve(a);
  ode_soln soln_a1 = growth_ode_solve(1.0);
  /** value at current time*/
  double D2_ct = soln.y2 / soln_a1.y2;
  double D1_ct = soln.y0 / soln_a1.y0;
  return 3 * D2_ct / D1_ct / D1_ct / 7;
}
