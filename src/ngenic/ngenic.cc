...
void ngenic::ngenic_displace_particles(void)
{
  ...
  mpi_printf("NGENIC: current time = %g\n", All.cf_atime);
  mpi_printf("NGENIC: vel_prefac1= %g  hubble_a=%g   fomega1=%g\n", vel_prefac1, All.cf_hubble_a, ngenic_f1_omega(All.cf_atime));
  mpi_printf("NGENIC: vel_prefac2= %g  hubble_a=%g   fomega2=%g\n", vel_prefac2, All.cf_hubble_a, ngenic_f2_omega(All.cf_atime));
  ...
  for(Type = MinType; Type <= MaxType; Type++)
#endif
    {
      ...
      double Phi2_factor_D2_over_D1Square = ngenic_Phi2fac(All.cf_atime);
      mpi_printf("NGENIC_2LPT: Exact solution of D2/D1/D1 %g \n", Phi2_factor_D2_over_D1Square);
      ...
      for(int axes = 0; axes < 3; axes++)
        {
          ...
          ngenic_readout_disp(disp, axes, Phi2_factor_D2_over_D1Square, Phi2_factor_D2_over_D1Square * vel_prefac2);
  ...
  if(ThisTask == 0)
    {
      ...
      FILE *fd = fopen(buf, "w");
      /** For new growth factor */  
      double gf = ngenic_growth_factor(0.00625, 1.0) / (1.0 / 0.00625);
